...
def flatten(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sum(([x] if type(x) == int else flatten(x) for x in lst),[])
    #consider the following logic
    # for every x in lst:
    # if x is an int type, just add x to the sum
    #else, execute flatten(x)
    #this is quite a simple syntax yet it's hard to follow the logic
...
def put_piece(board, max_rows, column, player):
    ...
    while max_rows > 0:
        if board[max_rows-1][column] == '-':
            row = max_rows-1
            tmp_board = replace_elem(sum(board[row:row+1],[]), column, player)
            final_board = replace_elem(board, row, tmp_board)
            break
        else:
            max_rows -= 1
            continue
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for i in range(0, max_cols - num_connect+1):
        tmp = sum(board[row:row+1],[])
        if win_row == tmp[i:i+num_connect]:
            return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    tmp = []
    for i in range(0, max_rows):
        tmp += get_piece(board, i, col)

    for i in range(0, max_cols - num_connect+1):
        if win_col == tmp[i:i+num_connect]:
            return True
    return False
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    True
    """
    if check_win_diagonal(board, max_rows, max_cols, num_connect,row, col, player):
            return True
