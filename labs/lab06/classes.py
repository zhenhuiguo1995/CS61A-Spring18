...
class Player(object):
    ...
    def unlock(self, place):
        ...
        for item in self.backpack:
            if type(item) == Key:
                key = item
        #how to update the locked value of the place. because the instance place is not given, it seems hard to access to instance attribute locked !
        if key == None:
            print(place,"can't be unlocked without a key!")
        #check the place's locked value if true open it, else print it is already unlocked! 
        elif self.place.get_neighbor(place).locked:
            key.use(place)
