...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        entrance -- The Place from which bees enter this Place.
        """
        self.name = name
        self.exit = exit
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does not do anything but protects its companions."""
    name = 'Wall'
    damage = 0
    implemented = True
    food_cost = 4
    ...
    def action(self, colony):
        ...
        if self.is_queen:
            ...
            while track_back.exit is not None:
                if track_back.ant is not None:
                    if track_back.ant not in self.ants_behind:
                ...
                track_back = track_back.exit
            if track_back.exit is None:
                if track_back.ant is not None:
                    if track_back.ant not in self.ants_behind:
                        track_back.ant.damage *= 2
                        self.ants_behind.append(track_back.ant)
